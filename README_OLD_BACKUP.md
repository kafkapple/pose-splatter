...
After training, evaluate the model on holdout views:
...
# Or direct execution (requires PYTHONPATH)
export PYTHONPATH="$(pwd):${PYTHONPATH}"
python scripts/training/evaluate_model.py \
  configs/baseline/markerless_mouse_nerf.json

# Results saved to: output/markerless_mouse_nerf/evaluation_metrics.json
```

#### Visualization

Generate renders and videos from trained model:
...
**Create videos from renders**:
```bash
# If visualization script fails to create videos
bash scripts/visualization/create_videos.sh
```

**Output locations**:
- Images: `output/markerless_mouse_nerf/renders/`
- Videos: `*.mp4` or `*.gif` (if libx264 unavailable)
- Point clouds: `output/markerless_mouse_nerf/pointclouds/`
