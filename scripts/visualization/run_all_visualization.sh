...
for view in 0 1 2 3 4 5; do
    ...
    python scripts/visualization/render_image.py $CONFIG 0 $view \
...
for frame in {0..29}; do
    ...
    python scripts/visualization/render_image.py $CONFIG $frame 0 \
...
for i in {0..23}; do
    angle=$(python -c "import math; print($i * 2 * math.pi / 24)")
    printf "  - Rendering angle %d (%.2f radians)\n" $i $angle
    python scripts/visualization/render_image.py $CONFIG 0 0 --angle_offset $angle \
...
python scripts/visualization/export_point_cloud.py --frame 0 2>&1 | grep -E "(Exporting|Point cloud|Saving|✓)"
echo ""
